						...
						if(!$image_in_column){
							...
							foreach($item_value->slideshow_inner_items as $inner_item_key => $inner_value){
								...
								if ($button_icon_position == 'left') {
									...
									$btn_content = ($button_icon) ? '<span class="sp-slider-btn-text">' . $btn_content . ' <span class="sp-slider-btn-icon"><i class="fa ' . $button_icon . '"></i></span></span>' : '<span class="sp-slider-btn-text">'.$btn_content.'</span>';
								}
								...
								if($content_type == 'text_content'){
									...
									$output .='<a id="'.$inner_uniqid.'" '.($button_target == '_blank' ? 'target="_blank"' : '').' class="sppb-sp-slider-button" href="'.$button_url.'" data-layer="true" data-animation=\'{'.$animation_settings.'}\'>';
									$output .= $btn_content;
									$output .='</a>';
			...
			foreach ($settings->slideshow_items as $item_key => $item_value) {
				...
				if(isset($item_value->slideshow_inner_items)){
					foreach($item_value->slideshow_inner_items as $inner_item_key => $inner_value){
						...
						$image_content_style .= (isset($inner_value->image_content_height) && $inner_value->image_content_height->md) ? 'height: ' . $inner_value->image_content_height->md . 'px;' : 'height:300px;';
